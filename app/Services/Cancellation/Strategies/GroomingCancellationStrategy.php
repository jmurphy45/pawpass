...
class GroomingCancellationStrategy implements OrderCancellationStrategy
{
    public function __construct(private readonly StripeService $stripe) {}

    public function supports(Order $order): bool
    {
        return $order->type === OrderType::Grooming;
    }
        ...
        foreach ($order->payments as $payment) {
            if ($payment->stripe_pi_id && $stripeAccountId) {
                try {
                    ...
                    Log::info('GroomingCancellationStrategy: Stripe cancel skipped', [
                        'order_id' => $order->id,
