<template>
  <PortalLayout>
    <div class="space-y-8">
      <div>
        <h1 class="text-2xl font-bold text-text-body">Subscribe</h1>
        <p class="text-sm text-text-muted mt-1">Get monthly credits delivered automatically</p>
      </div>
              ...
              <div class="flex-1 min-w-0">
                <div class="flex items-baseline gap-3 flex-wrap">
                  ...
                  <p class="text-2xl font-black text-text-body">
                    ...
                    <span class="text-sm font-normal text-text-muted">/ month</span>
                  </p>
                </div>
                ...
                <ul class="mt-2.5 space-y-1.5">
                  <li class="flex items-center gap-2 text-sm text-text-muted">
                    ...
                    {{ pkg.credits ? `${pkg.credits} credits / month` : 'Unlimited visits / month' }}
                  </li>
                  <li class="flex items-center gap-2 text-sm text-text-muted">
                    <svg class="h-4 w-4 text-green-500 shrink-0" fill="currentColor" viewBox="0 0 20 20">
                      <path fill-rule="evenodd" d="M10 18a8 8 0 1 0 0-16 8 8 0 0 0 0 16Zm3.857-9.809a.75.75 0 0 0-1.214-.882l-3.483 4.79-1.88-1.88a.75.75 0 1 0-1.06 1.061l2.5 2.5a.75.75 0 0 0 1.137-.089l4-5.5Z" clip-rule="evenodd" />
          ...
          <div class="card-padded sticky top-24 space-y-5">
            ...
            <div v-if="selectedPackage" class="rounded-lg bg-surface-subtle px-4 py-3 text-sm">
              <div class="flex items-center justify-between">
                ...
                <span class="font-semibold text-text-body">${{ (selectedPackage.price_cents / 100).toFixed(2) }}/mo</span>
              </div>
              <p class="text-xs text-text-muted mt-1">Billed monthly · cancel anytime</p>
            </div>
            ...
            <button
              ...
              :style="{ backgroundColor: accentColor }"
            >
              <svg v-if="subscribing" class="h-4 w-4 animate-spin" fill="none" viewBox="0 0 24 24">
                <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"/>
                <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"/>
              </svg>
              {{ subscribing ? 'Processing…' : selectedPackage ? `Subscribe — $${(selectedPackage.price_cents / 100).toFixed(2)}/mo` : 'Subscribe' }}
            </button>
