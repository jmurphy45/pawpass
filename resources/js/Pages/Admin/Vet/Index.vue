  ...
  <AdminLayout>
    <div class="space-y-5">
      ...
      <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          <h1 class="text-2xl font-bold text-text-body">Vet Appointments</h1>
          <p class="text-sm text-text-muted mt-0.5">Manage clinical and vet visits</p>
        </div>
        ...
        <table class="min-w-full divide-y divide-border-warm">
          <thead class="bg-surface-muted">
            <tr>
              ...
              <th class="px-4 py-3 text-left text-xs font-medium text-text-muted uppercase tracking-wider">Reason</th>
          ...
          <tbody class="divide-y divide-border-warm">
            ...
            <tr v-for="appt in props.appointments.data" :key="appt.id" class="hover:bg-surface-muted/50 transition">
              ...
              <td class="px-4 py-3 text-sm text-text-muted max-w-48 truncate">{{ appt.vet_detail?.reason ?? '—' }}</td>
              ...
              <td class="px-4 py-3 text-sm text-text-muted">{{ appt.vet_detail ? formatCents(appt.vet_detail.price_cents) : '—' }}</td>
              <td class="px-4 py-3 text-right">
                <Link :href="route('admin.vet.appointments.show', { appointment: appt.id })" class="text-indigo-600 hover:text-indigo-800 text-sm font-medium">View</Link>
              </td>
            </tr>
          </tbody>
        </table>
      ...
      <div v-if="showCreate" class="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        <div class="relative bg-white rounded-2xl shadow-xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
          <div class="px-6 py-5 border-b border-border-warm flex items-center justify-between">
            <h2 class="text-lg font-semibold text-text-body">New Vet Appointment</h2>
          ...
          <form @submit.prevent="submitCreate" class="px-6 py-5 space-y-4">
            ...
            <div>
              <label class="block text-sm font-medium text-text-body mb-1">Reason <span class="text-red-500">*</span></label>
              <input v-model="form.reason" type="text" class="cr-input" placeholder="Annual check-up, vaccination, etc." />
              <p v-if="form.errors.reason" class="cr-error">{{ form.errors.reason }}</p>
            </div>
            ...
            <div class="grid grid-cols-2 gap-3">
              ...
              <div>
                ...
                <input v-model.number="form.duration_mins" type="number" min="1" class="cr-input" placeholder="30" />
              </div>
            </div>
            ...
            <div v-if="props.resources.length > 0">
              <label class="block text-sm font-medium text-text-body mb-1">Exam Room</label>
...
function applyFilters() {
  router.get(route('admin.vet.appointments.index'), { ...filters.value }, { preserveState: true, replace: true });
}
...
function submitCreate() {
  form.post(route('admin.vet.appointments.store'), {
