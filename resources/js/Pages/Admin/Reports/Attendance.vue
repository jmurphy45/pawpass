        ...
        <table class="min-w-full divide-y divide-gray-200 text-sm">
          <thead class="bg-gray-50">
            <tr>
              ...
              <th class="px-4 py-3 text-right font-medium text-gray-600">Unique Dogs</th>
            </tr>
          </thead>
          <tbody class="divide-y divide-gray-100">
            <tr v-if="rows.length === 0">
              <td colspan="3" class="px-4 py-6 text-center text-gray-400">No attendance data for selected range.</td>
            </tr>
            <tr v-for="row in rows" :key="row.period" class="hover:bg-gray-50">
              <td class="px-4 py-3 text-gray-900">{{ row.period }}</td>
              <td class="px-4 py-3 text-right text-gray-900">{{ row.checkins }}</td>
              <td class="px-4 py-3 text-right text-gray-700">{{ row.unique_dogs }}</td>
            </tr>
          </tbody>
          <tfoot v-if="rows.length > 0" class="bg-gray-50 font-semibold">
            <tr>
              <td class="px-4 py-3 text-gray-900">Total</td>
              <td class="px-4 py-3 text-right">{{ totalCheckins }}</td>
...
const totalCheckins = computed(() => props.rows.reduce((s, r) => s + r.checkins, 0));

const csvUrl = computed(() =>
  `${route('admin.reports.attendance')}?from=${localFrom.value}&to=${localTo.value}&group_by=${localGroupBy.value}&format=csv`
);

function applyFilters(): void {
  router.get(route('admin.reports.attendance'), {
