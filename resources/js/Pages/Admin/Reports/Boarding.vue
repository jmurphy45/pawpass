  ...
  <AdminLayout>
    <div class="space-y-6">
      <div class="flex items-center justify-between">
        <h1 class="text-2xl font-bold text-gray-900">Boarding Revenue</h1>
          ...
          <table class="min-w-full divide-y divide-gray-200 text-sm">
            ...
            <tbody class="divide-y divide-gray-100">
              <tr v-if="rows.length === 0">
                <td colspan="5" class="px-4 py-6 text-center text-gray-400">No boarding revenue for selected range.</td>
              </tr>
...
const csvUrl = computed(() =>
  `${route('admin.reports.boarding')}?from=${localFrom.value}&to=${localTo.value}&group_by=${localGroupBy.value}&format=csv`
);
...
function applyFilters(): void {
  router.get(route('admin.reports.boarding'), {
