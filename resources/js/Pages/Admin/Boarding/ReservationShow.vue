  ...
  <AdminLayout>
    <div class="space-y-6">
      ...
      <div class="flex items-center gap-3 flex-wrap">
        ...
        <AppBadge :color="statusBadgeColor(reservation.status)">{{ reservation.status }}</AppBadge>
      </div>

      <!-- Status Actions (non-checkout statuses) -->
      <AppCard v-if="availableActions.length > 0" class="overflow-hidden">
        ...
        <div v-if="confirmingAction" class="flex items-center gap-3 text-sm">
          ...
          <AppButton variant="primary" size="sm" @click="executeAction">Confirm</AppButton>
          <AppButton variant="secondary" size="sm" @click="confirmingAction = null">Cancel</AppButton>
        </div>
      ...
      <AppCard v-if="reservation.status === 'checked_in'" class="overflow-hidden">
        ...
        <div class="p-5 space-y-3">
          <div>
            ...
            <input v-model="checkoutDate" type="date" class="w-full rounded-lg border border-border-warm px-3 py-2.5 text-sm bg-white text-text-body outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition w-48" :min="checkoutMinDate" />
          </div>
        ...
        <div class="lg:col-span-2 space-y-6">
          ...
          <AppCard class="overflow-hidden">
            ...
            <form @submit.prevent="submitReportCard" class="space-y-2 pt-2 border-t border-border">
              ...
              <input v-model="cardForm.report_date" type="date" class="w-full rounded-lg border border-border-warm px-3 py-2.5 text-sm bg-white text-text-body outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition" required />
              <textarea v-model="cardForm.notes" class="w-full rounded-lg border border-border-warm px-3 py-2.5 text-sm bg-white text-text-body outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition h-24 resize-none" placeholder="How was the stay today?" required />
          ...
          <AppCard class="overflow-hidden">
            ...
            <form @submit.prevent="addAddon" class="flex gap-2 pt-2 border-t border-border">
              <select v-model="addonForm.addon_type_id" class="w-full rounded-lg border border-border-warm px-3 py-2.5 text-sm bg-white text-text-body outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition flex-1">
                <option value="">Select add-on…</option>
                <option v-for="at in addonTypes" :key="at.id" :value="at.id">
                  {{ at.name }} (${{ (at.price_cents / 100).toFixed(2) }})
                </option>
              </select>
              <AppButton type="submit" variant="primary" :disabled="!addonForm.addon_type_id">Add</AppButton>
            </form>
            </div>
          </AppCard>
        </div>
          ...
          <AppCard class="overflow-hidden">
            ...
            <dl v-else class="space-y-2 text-sm">
              ...
              <div class="flex justify-between">
                ...
                <dd>
                  <AppBadge :color="depositBadgeColor">{{ depositStatus }}</AppBadge>
                </dd>
              </div>
...
const depositBadgeColor = computed(() => {
  if (depositStatus.value === 'Captured') return 'green';
  if (depositStatus.value === 'Released') return 'gray';
  if (depositStatus.value === 'Authorized') return 'blue';
  return 'yellow';
});
...
function statusBadgeColor(status: unknown): string {
  if (status === 'pending') return 'yellow';
  if (status === 'confirmed') return 'blue';
  if (status === 'checked_in') return 'green';
  if (status === 'checked_out') return 'gray';
