  ...
  <AdminLayout>
    <div class="space-y-5 max-w-3xl">
      ...
      <div class="flex items-center justify-between gap-3">
        <div>
          <Link :href="route('admin.grooming.appointments.index')" class="text-sm text-indigo-600 hover:underline mb-1 inline-block">← Grooming Appointments</Link>
          <h1 class="text-2xl font-bold text-text-body">
            Grooming — {{ props.appointment.dog?.name ?? 'Unknown Dog' }}
          </h1>
      ...
      <AppCard>
        ...
        <dl class="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-4 text-sm">
          ...
          <div v-if="detail">
            <dt class="text-text-muted font-medium">Service</dt>
            <dd class="mt-0.5 text-text-body">{{ detail.service_name }}</dd>
          </div>
          <div v-if="detail">
          ...
          <div v-if="detail?.resource">
            <dt class="text-text-muted font-medium">Grooming Bay</dt>
        ...
        <div v-if="showEditModal" class="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div class="relative bg-white rounded-2xl shadow-xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
            ...
            <form @submit.prevent="submitEdit" class="px-6 py-5 space-y-4">
              <div>
                <label class="block text-sm font-medium text-text-body mb-1">Service</label>
                <input v-model="editForm.service_name" type="text" class="cr-input" />
                <p v-if="editForm.errors.service_name" class="cr-error">{{ editForm.errors.service_name }}</p>
              </div>
...
const detail = computed(() => props.appointment.grooming_detail ?? null);
...
const editForm = useForm({
  service_name: detail.value?.service_name ?? '',
...
function confirmAppt() {
  confirmForm.post(route('admin.grooming.appointments.confirm', { appointment: props.appointment.id }));
}

function submitEdit() {
  editForm.patch(route('admin.grooming.appointments.update', { appointment: props.appointment.id }), {
    onSuccess: () => { showEditModal.value = false; },
  });
}

function submitCancel() {
  cancelForm.post(route('admin.grooming.appointments.cancel', { appointment: props.appointment.id }), {
