...
class ReportServiceTest extends TestCase
{
    ...
    public function test_revenue_includes_tax_and_processing_fee_totals(): void
    {
        $customer = Customer::factory()->create(['tenant_id' => $this->tenant->id]);
        $package = Package::factory()->create(['tenant_id' => $this->tenant->id]);
        ...
        Order::factory()->create([
            ...
            'processing_fee_amount_cents' => 290,
            'created_at' => '2026-02-10 10:00:00',
        ]);

        $result = $this->service->revenue($this->tenant->id, '2026-02-01', '2026-02-28 23:59:59', 'month');

        $this->assertCount(1, $result);
        $this->assertArrayHasKey('tax_total', $result[0]);
        $this->assertArrayHasKey('processing_fee_total', $result[0]);
        $this->assertEquals(8.0, $result[0]['tax_total']);
        $this->assertEquals(2.9, $result[0]['processing_fee_total']);
    }

    public function test_revenue_uses_stored_platform_fee_amount_cents(): void
    {
        $customer = Customer::factory()->create(['tenant_id' => $this->tenant->id]);
        $package = Package::factory()->create(['tenant_id' => $this->tenant->id]);

        // platform_fee_pct=5% would give $5 fee, but stored amount is $0 (founders cap applied)
        Order::factory()->create([
            'tenant_id' => $this->tenant->id,
            'customer_id' => $customer->id,
            'package_id' => $package->id,
        ...
        $result = $this->service->revenue($this->tenant->id, '2026-03-01', '2026-03-31 23:59:59', 'month');

        $this->assertCount(1, $result);
        $this->assertEquals(0.0, $result[0]['fee']);
        $this->assertEquals(100.0, $result[0]['net']);
    }

    // ----- Promotions -----

    public function test_promotions_aggregates_redemptions(): void
    {
        ...
        DB::table('promotion_redemptions')->insert([
            ['id' => (string) \Illuminate\Support\Str::ulid(), 'tenant_id' => $this->tenant->id, 'promotion_id' => $promotion->id, 'order_id' => $order->id, 'customer_id' => $customer->id, 'discount_amount_cents' => 1000, 'original_amount_cents' => 10000, 'created_at' => '2026-01-10 10:00:00'],
            ['id' => (string) \Illuminate\Support\Str::ulid(), 'tenant_id' => $this->tenant->id, 'promotion_id' => $promotion->id, 'order_id' => $order->id, 'customer_id' => $customer->id, 'discount_amount_cents' => 500, 'original_amount_cents' => 5000, 'created_at' => '2026-01-15 10:00:00'],
        ]);

        $result = $this->service->promotions($this->tenant->id, '2026-01-01', '2026-01-31 23:59:59');

        $this->assertCount(1, $result);
        $this->assertEquals('SAVE10', $result[0]['code']);
        $this->assertEquals(2, $result[0]['redemptions']);
        $this->assertEquals(1500, $result[0]['total_discount_cents']);
    }

    public function test_promotions_excludes_other_tenant(): void
    {
        $other = Tenant::factory()->create(['slug' => 'other-promo', 'status' => 'active']);
        $promotion = \App\Models\Promotion::factory()->create(['tenant_id' => $other->id, 'code' => 'OTHER']);
        $customer = Customer::factory()->create(['tenant_id' => $other->id]);
        $package = Package::factory()->create(['tenant_id' => $other->id]);

        app()->forgetInstance('current.tenant.id');
        app()->instance('current.tenant.id', $other->id);
        $order = Order::factory()->create(['tenant_id' => $other->id, 'customer_id' => $customer->id, 'package_id' => $package->id, 'status' => 'paid']);
        app()->forgetInstance('current.tenant.id');
        app()->instance('current.tenant.id', $this->tenant->id);

        DB::table('promotion_redemptions')->insert([
            ['id' => (string) \Illuminate\Support\Str::ulid(), 'tenant_id' => $other->id, 'promotion_id' => $promotion->id, 'order_id' => $order->id, 'customer_id' => $customer->id, 'discount_amount_cents' => 500, 'original_amount_cents' => 5000, 'created_at' => '2026-01-10 10:00:00'],
        ]);
    ...
    public function test_boarding_revenue_filters_by_type(): void
    {
        $customer = Customer::factory()->create(['tenant_id' => $this->tenant->id]);
        $package = Package::factory()->create(['tenant_id' => $this->tenant->id]);
        ...
        Order::factory()->create([
            ...
            'total_amount' => '50.00',
            'platform_fee_pct' => '5.00',
            'platform_fee_amount_cents' => 250,
            'created_at' => '2026-01-20 10:00:00',
        ]);

        $result = $this->service->boardingRevenue($this->tenant->id, '2026-01-01', '2026-01-31 23:59:59', 'month');

        $this->assertCount(1, $result);
        $this->assertEquals(200.0, $result[0]['gross']);
        $this->assertEquals(10.0, $result[0]['fee']);
        $this->assertEquals(190.0, $result[0]['net']);
        $this->assertEquals(1, $result[0]['orders']);
    }

    // ----- Outstanding Balances -----

    public function test_outstanding_balances_returns_customers_with_debt(): void
    {
        $debtor = Customer::factory()->create(['tenant_id' => $this->tenant->id, 'name' => 'Owes Money', 'outstanding_balance_cents' => 5000]);
        $current = Customer::factory()->create(['tenant_id' => $this->tenant->id, 'name' => 'Paid Up', 'outstanding_balance_cents' => 0]);

        $result = $this->service->outstandingBalances($this->tenant->id);

        $names = array_column($result, 'customer_name');
        $this->assertContains('Owes Money', $names);
        $this->assertNotContains('Paid Up', $names);

        $row = collect($result)->firstWhere('customer_name', 'Owes Money');
        $this->assertEquals(5000, $row['outstanding_balance_cents']);
    }

    public function test_outstanding_balances_excludes_other_tenant(): void
    {
        $other = Tenant::factory()->create(['slug' => 'other-bal', 'status' => 'active']);
        Customer::factory()->create(['tenant_id' => $other->id, 'outstanding_balance_cents' => 9999]);
